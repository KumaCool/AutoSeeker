...
```bash
uv run autoseeker auth import /path/to/cookies.json
uv run autoseeker auth check
uv run autoseeker config show
```
...
- macOS：保留 launchd 模板。
- 两者都调用同一个 `autoseeker collect`，不复制业务逻辑。

安装、卸载和修改调度必须显式执行。定时任务缺少有效 Cookie 时应快速失败，不自动打开浏览器。

Linux 用户单元可用 `./deploy/systemd/install-user-timer.sh` 生成；脚本只生成文件并打印启用命令，不自动调用 `systemctl --user enable --now`。
